...
<h1 class="page-title">Product Testimonial</h1>
<table id="product_table" class="cell-border" style="width:100%">
        <thead>
            <tr>
                <th>Sr. No</th>
                <th>Product Id</th>
...
global $wpdb;
 $pre = $wpdb->prefix;
 $product_testimonial = 'rvw_product_form'; 
 $sql_data = $wpdb->get_results( "SELECT * FROM $product_testimonial");
 $i = 0;
...
<div class="modal fade cs-feedback_model" id="myModal" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-header">
          <h3>Product Teatimonial Details</h3> <button type="button" class="close" data-dismiss="modal">&times;</button>
        </div>
    ...
    jQuery('.cs_view').click(function(){
        ...
        jQuery('.feedback_data_display').hide();
         ...
         jQuery.ajax({
				     ...
				     type: "POST",
				     data:{ 
				    		    action: 'get_product_data', id:row_id },
				    			success: function(data) {
				    			    jQuery('.feedback_data_display').show();
				    				jQuery('.feedback_data_display').html(data.product_data);
