...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
 <?php 
 // echo strtotime('2023-05-18');
 ?>
<div class="feedback-page">
<h1 class="page-title">No Click Star Feedback</h1>
<table id="feedfack_table1" class="cell-border" style="width:100%">
        <thead>
            <tr>
                <th>Sr. No</th>
                <th>Full Name</th>    
                ...
                <th>Stars</th>
                <th>Created Date</th>
                <th>Email after 2 days</th>
                <th>Email after 5 days</th>
                <th>Action</th>
            </tr>
        </thead>
        <tbody>
...
global $wpdb;
 ...
 $sql_data = $wpdb->get_results( "SELECT * FROM $feedback_table");
 $i = 0;
 foreach ($sql_data as $result_dataa) {   
    $i++;
    $timestamp = strtotime($result_dataa->created_date);
    $created_date = date("Y-m-d", $timestamp);
    if( $result_dataa->site_domain == $site_domain && empty($result_dataa->star_value)){
 ?>
            ...
            <tr>
                ...
                <td><?php if($result_dataa->email_after_2_days == 'success'){ echo '<span style="color:green">success</span>'; }else{ echo '<span style="color:red">Pending</span>'; } ?></td>
                <td><?php if($result_dataa->email_after_5_days == 'success'){ echo '<span style="color:green">success</span>'; }else{ echo '<span style="color:red">Pending</span>'; } ?></td>
...
jQuery(document).ready(function () {
    jQuery('#feedfack_table1').DataTable();
});
</script>
<script>
